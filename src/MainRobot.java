...
// 		//Turn again
		afterBoomDistance(decision,3000); //2nd variable is time after boom lift				
		//Turn in canyon and before move out of canyon
		canyonMove(decision);
		r.sleep(1000);
////		
...
//		System.out.println(getConductivity());
	...
	public static void firstTurn(int decision) {
		if(decision == 0) {
			...
			TurnValues(right, 250, 250,1470); 
		}
		else {
			TurnValues(left, -250,-250, 1280);
		}
	}
	...
	public static void canyonMove(int decision) {
		if(decision == 0) {
			...
			r.sleep(2000);
			//TUrns right
			TurnValues(right, 250,250,1575); //1350
		}
...
//		}
		...
		//IS if doesn't go directly, and needs to turn multiples times
			...
			if(decision == 0) {
				...
				TurnValues(left, -250,-250,1230); //First LEft
				...
				TurnValues(right, 250,250,1380); //TEST
				//This is what happens if it goes not direct
				boolean secondOption = true;
				MotorsIndefinite();
				while(secondOption) {
				...
				r.sleep(1000);
				TurnValues(left, -250, -250, 1230); //TEST
				r.sleep(1000);
				MotorsIndefinite();
				secondOption = true;
				while(secondOption) {
	...
	public static void backItUp(int decision) {
		if(decision == 0) {
			r.sleep(1000);
			TurnValues(right,-250,-250,2400); //180
			r.sleep(1000);
		...
		//	TurnValues(left,-250,-250,1300);
			...
			r.sleep(1000);
			r.runMotor(right, 190, left, -180, 1800); //reverse 180
			r.sleep(1000);
			r.runMotor(right, rightSpeed, left, leftSpeed, 300);
			r.sleep(1000);
	...
	public static void acrossBridge(int decision) {
		...
		if(decision == 0) {
			MotorsRampBridge(8220);
		}
		else {
			MotorsRampBridge(8240);
		}
	...
	public static void sandBoxDistance(int decision) {
		if(decision == 0) {
			r.sleep(1000);
			TurnValues(right, 250, 250, 1350);
			r.sleep(1000);
			MotorsIndefiniteNew();
			boolean x = true;
			while(x) {
				if(Ping(FRONT_PING_PIN) < 6) {
					StopMotors();
					x= false;
				}
			}
...
//		}
		...
		conCode = (-.11 * conCode) + 111.16; //107.16
		return conCode;
	}
	...
	public static void Temperature() {
		...
		System.out.println("Temp of Covered: " + ((tempCovered - 3))); //DIsplays covdered -5
	...
	public static void TurnRamp (int motor) {
		if(motor == right) {
			r.runMotor(right, 250, left, 250, 1090); //35-
		}
		else {
			r.runMotor(right, -250, left, -250, 855); //870
		}
	}
